"""OCR SoulWorker 勳章 (medals) through an OpenAI-compatible vision endpoint.

Sends each medal screenshot to a vision model and returns the structured fields
勳章等級 / 勳章名稱 / 效果 as JSON.

A medal row has three parts:
  - left  : a coloured wing icon whose colour is the medal *grade* (等級) —
            紫色(purple)=TB、紅色(red)=GB、黃色(yellow)=MB.
  - middle: the medal name, e.g. "攻擊型 SD：鷹眼".
  - right : the effect text, e.g. "命中度 +31" or a longer conditional buff.
...
Auth     : read from a .env file at the repo root (or any parent dir). Any of
    ...
    uv run tools/medal_ocr.py medal.png
    uv run tools/medal_ocr.py ./tools/captures/SD/ --save-dir ./medals/SD
    uv run tools/medal_ocr.py medal.png --model gpt-5.4
    uv run tools/medal_ocr.py medal.png --dry-run     # build the request, don't call
"""
...
SYSTEM_PROMPT = """你是《靈魂行者》(SoulWorker) 勳章的 OCR 助手。辨識使用者提供的勳章圖片，\
只輸出「一個」JSON 物件，不要附加任何說明文字或 markdown 圍欄。

每張圖片由左到右有三個區塊：左邊是一個翅膀圖示、中間是勳章名稱、右邊是效果文字。欄位與規則如下：

勳章等級: string，依左邊「翅膀圖示的顏色」判斷，只能是 "TB"、"GB" 或 "MB" 之一 —\
紫色=TB、紅色=GB、黃色(金色)=MB。
勳章名稱: string，中間的勳章名稱，完整保留，例如「攻擊型 SD：鷹眼」。
效果: string，右邊的效果敘述，完整保留（原文換行用空格代替），例如「命中度 +31」。

輸出格式（鍵名需完全相同）：
{"勳章等級": "", "勳章名稱": "", "效果": ""}
無法辨識的欄位填 ""。"""

USER_TEXT = "OCR 這張勳章，依系統指示輸出 JSON。"
...
def prepare_image(path: Path, max_px: int = DEFAULT_MAX_PX) -> str:
    ...
    Medals are screenshots, so we flatten to RGB, cap the longest side at max_px
...
def normalize(data: dict) -> dict:
    """Keep exactly FIELDS as strings; uppercase the grade and blank it if invalid."""
    out: dict = {}
    for k in FIELDS:
        v = data.get(k)
        out[k] = "" if v is None else str(v).strip()
    grade = out["勳章等級"].upper()
    out["勳章等級"] = grade if grade in GRADES else ""
    return out
...
# OCR
# --------------------------------------------------------------------------- #
def ocr_medal(client: OpenAI, model: str, path: Path, max_px: int = DEFAULT_MAX_PX) -> dict:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OCR SoulWorker 勳章 via an OpenAI-compatible vision endpoint.")
    p.add_argument("images", nargs="*", help="Image file(s) or folder(s) of medal images.")

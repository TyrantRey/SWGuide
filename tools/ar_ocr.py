...
卡名 / 星數 / 等級 / 稀有度 / 類型 / 能力名字 / 能力描述 as JSON.
...
Auth     : read from a .env file at the repo root (or any parent dir). Any of
    ...
    uv run tools/ar_ocr.py card.png
    uv run tools/ar_ocr.py ./cards/ --save-dir ./ar-cards
    uv run tools/ar_ocr.py card.png --model gpt-5.4
    uv run tools/ar_ocr.py card.png --dry-run     # build the request, don't call
"""
...
FIELDS = ["卡名", "星數", "等級", "稀有度", "類型", "能力名字", "能力描述"]
INT_FIELDS = {"星數", "等級"}

SYSTEM_PROMPT = """你是《靈魂行者》(SoulWorker) AR 卡的 OCR 助手。辨識使用者提供的 AR 卡圖片，\
只輸出「一個」JSON 物件，不要附加任何說明文字或 markdown 圍欄。欄位與規則如下：

卡名: string，卡片名稱。
星數: int，星星的「數量」。
等級: int，強化等級，只能是 0、1 或 2（對應 +0 / +1 / +2）。
稀有度: string，依星星「顏色」判斷 —「神秘」=紫色星星、「隱藏」=淺藍色星星、「一般」=黃色星星。
類型: string —「主動」(圖中有 active 字眼)、「被動」(有 passive 字眼)、「核心」(有 core 字眼)。
能力名字: string，能力／技能的名稱。
能力描述: string，能力的完整描述文字（保留原文換行可用空格代替）。

輸出格式（鍵名需完全相同）：
{"卡名": "", "星數": 0, "等級": 0, "稀有度": "", "類型": "", "能力名字": "", "能力描述": ""}
無法辨識的欄位：字串填 ""、數字填 0。"""

USER_TEXT = "OCR 這張 AR 卡，依系統指示輸出 JSON。"
...
def normalize(data: dict) -> dict:
    """Keep exactly FIELDS, coerce the int fields, default the rest."""
    out: dict = {}
    for k in FIELDS:
        v = data.get(k)
        if k in INT_FIELDS:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OCR SoulWorker AR cards via an OpenAI-compatible vision endpoint.")
    p.add_argument("images", nargs="*", help="Image file(s) or folder(s) of AR-card images.")
